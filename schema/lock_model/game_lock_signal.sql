...
-- (the direct analog of the earlier "no LOCK below 35" check, now
-- correct for a per-player bar instead of a flat one)
SELECT COUNT(*) AS violations
FROM game_lock_signal
WHERE lock_signal = 'LOCK' AND fantasy_score < lock_bar;

-- Overall distribution -- expect LOCK's share to drop noticeably vs the
-- old flat-35 version, since high-average players now need a real spike,
-- not just an ordinary good game
...
-- Spot check Jokić's week 5, 2024-25 -- his lock_bar should sit well
-- above 35 (his 2024-25 avg was ~70.77, stddev ~17.44 -> lock_bar ~79.5),
-- meaning most of his games should now show HOLD/PASS, not automatic
-- LOCK, unless a game is a genuine outlier even by HIS standards
SELECT p.full_name, gfsw.game_date, gfsw.games_remaining_in_week,
       gfsw.fantasy_score, gfsw.lock_bar, gfsw.lock_signal
