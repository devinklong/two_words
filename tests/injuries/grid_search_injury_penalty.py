...
# validated via the original grid search (schema/lock_model docs, 8/9/26).
POLICY_SQL = """
WITH base AS (
    SELECT
...
scored AS (
    SELECT
        *,
        GREATEST(35, player_avg + 0.5 * player_std)
            + CASE WHEN is_return_game THEN %(penalty)s ELSE 0 END AS lock_bar,
        CASE
            WHEN fantasy_score >= GREATEST(35, player_avg + 0.5 * player_std)
